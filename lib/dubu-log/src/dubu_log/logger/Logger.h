...
protected:
  void InternalLog(
      LogLevel, const std::string&, uint32_t, const std::string&, const std::string&) override {}
};

class Logger {
public:
  static void RegisterLogger(std::unique_ptr<ILogger>&& logger) { ourLogger = std::move(logger); }
...
std::enable_if_t<std::is_base_of_v<ILogger, T>> Register(Args&&... args) {
  dubu::log::internal::Logger::RegisterLogger(std::make_unique<T>(std::forward<Args>(args)...));
}
}  // namespace dubu::log
